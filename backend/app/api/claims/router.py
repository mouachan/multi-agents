...
from app.api.claims import schemas
...
async def get_claim_decision(
    ...
    return await get_decision(
        ...
        id_column="claim_id",
        response_schema=schemas.ClaimDecisionResponse,
        entity_label="claim",
        extra_fields_fn=lambda d: {
            "relevant_policies": d.relevant_policies,
            "similar_claims": d.similar_claims,
            "user_contract_info": d.user_contract_info,
        },
    )
...
async def view_claim_document(
    ...
    """View claim document PDF. Accepts UUID or claim_number (e.g. CLM-2024-0019). Use lang=en for English version."""
    try:
        try:
        ...
        if lang == "en" and metadata.get("document_path_en"):
            ...
            fallback = claim.document_path

        doc = get_document(claim.claim_number, fallback_path=fallback)
        if not doc:
