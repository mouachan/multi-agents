...
from app.api.tenders import schemas
from app.api.shared.schemas import ProcessingStepLog
from app.api.shared.decision_service import get_decision
...
async def get_tender_decision(
    ...
    return await get_decision(
        ...
        id_column="tender_id",
        response_schema=schemas.TenderDecisionResponse,
        entity_label="tender",
        extra_fields_fn=lambda d: {
            "risk_analysis": d.risk_analysis,
            "similar_references": d.similar_references,
            "historical_ao_analysis": d.historical_ao_analysis,
            "internal_capabilities": d.internal_capabilities,
        },
    )
...
async def view_tender_document(
    ...
    """View tender document PDF. Accepts UUID or tender_number. Use lang=en for English version."""
    try:
        try:
        ...
        # Determine document path based on language
        metadata = tender.tender_metadata or {}
        if lang == "en" and metadata.get("document_path_en"):
            fallback = metadata["document_path_en"]
        else:
            fallback = tender.document_path

        doc = get_document(tender.tender_number, fallback_path=fallback)
        if not doc:
