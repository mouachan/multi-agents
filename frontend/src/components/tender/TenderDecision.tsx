...
export default function TenderDecision({ tender, decision }: TenderDecisionProps) {
  ...
  return (
    <div className="bg-white shadow rounded-lg p-6">
      <h3 className="text-xl font-bold text-gray-900 mb-4">{t('tenderDecision.title')}</h3>

      {isManualReview ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <div className="border border-gray-200 rounded-lg p-4">
            <p className="text-sm font-medium text-gray-500 mb-2">{t('tenderDecision.systemDecisionInitial')}</p>
            ...
            {decision.initial_confidence != null && (
              <p className="text-sm text-gray-600 mt-2">
                {t('tenderDecision.confidence')}: <span className="font-medium">{(decision.initial_confidence * 100).toFixed(1)}%</span>
              </p>
            )}
          </div>
          ...
          <div className={`border-2 rounded-lg p-4 ${
            ...
            <p className="text-sm font-medium text-gray-700 mb-2">{t('tenderDecision.finalDecision')}</p>
            {decision.final_decision ? (
              <>
                ...
                {decision.final_decision_by_name && (
                  <p className="text-sm text-gray-700 mt-2">{t('tenderDecision.by')}: {decision.final_decision_by_name}</p>
                )}
              ...
              <p className="text-lg text-orange-600 font-medium">{t('tenderDecision.awaitingReview')}</p>
            )}
          </div>
        </div>
      ) : (
        <div className="mb-6">
          <div className={`border-2 rounded-lg p-6 ${getDecisionBorder(decision.initial_decision)}`}>
            <p className="text-sm font-medium text-gray-500 mb-2">{t('tenderDecision.systemDecision')}</p>
            ...
            {decision.win_probability_estimate != null && (
              <p className="text-sm text-gray-600 mt-1">
                {t('tenderDecision.winProbability')}: <span className="font-medium text-amber-700">{(decision.win_probability_estimate * 100).toFixed(1)}%</span>
              </p>
            )}
            {decision.estimated_margin_percentage != null && (
              <p className="text-sm text-gray-600 mt-1">
                {t('tenderDecision.estimatedMargin')}: <span className="font-medium text-amber-700">{decision.estimated_margin_percentage.toFixed(1)}%</span>
              </p>
            )}
          </div>
        </div>
      ...
      {decision.risk_analysis && (
        <div className="mt-6">
          <h4 className="text-lg font-semibold text-gray-900 mb-3">{t('tenderDecision.riskAnalysis')}</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {decision.risk_analysis.technical && (
              <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-xs font-semibold text-blue-800 mb-1">{t('tenderDecision.technical')}</p>
                <p className="text-sm text-gray-700">{decision.risk_analysis.technical}</p>
              </div>
            )}
            {decision.risk_analysis.financial && (
              <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
                <p className="text-xs font-semibold text-green-800 mb-1">{t('tenderDecision.financial')}</p>
                <p className="text-sm text-gray-700">{decision.risk_analysis.financial}</p>
              </div>
            )}
            {decision.risk_analysis.resource && (
              <div className="p-3 bg-purple-50 border border-purple-200 rounded-lg">
                <p className="text-xs font-semibold text-purple-800 mb-1">{t('tenderDecision.resources')}</p>
                <p className="text-sm text-gray-700">{decision.risk_analysis.resource}</p>
              </div>
            )}
            {decision.risk_analysis.competition && (
              <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
                <p className="text-xs font-semibold text-red-800 mb-1">{t('tenderDecision.competition')}</p>
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          {decision.strengths && decision.strengths.length > 0 && (
            <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
              <h4 className="text-sm font-semibold text-green-800 mb-2">{t('tenderDecision.strengths')}</h4>
          ...
          {decision.weaknesses && decision.weaknesses.length > 0 && (
            <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
              <h4 className="text-sm font-semibold text-red-800 mb-2">{t('tenderDecision.weaknesses')}</h4>
              <ul className="list-disc list-inside text-sm text-gray-700 space-y-1">
      ...
      {decision.recommended_actions && decision.recommended_actions.length > 0 && (
        <div className="mt-6 p-4 bg-amber-50 border border-amber-200 rounded-lg">
          <h4 className="text-sm font-semibold text-amber-800 mb-2">{t('tenderDecision.recommendedActions')}</h4>
          <ul className="list-disc list-inside text-sm text-gray-700 space-y-1">
      ...
      {decision.llm_model && (
        <div className="mt-4">
          <p className="text-sm text-gray-600">{t('tenderDecision.llmModel')}: {decision.llm_model}</p>
        </div>
      )}
    </div>
  )
